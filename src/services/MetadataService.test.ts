...
describe("MetadataService", () => {
	...
	describe("wikilink formatting", () => {
		const formatAsWikilinks = (values: string[], fieldKey: any) =>
			(metadataService as any).formatAsWikilinks(values, fieldKey);

		beforeEach(() => {
			mockSettings.fieldsSettings.authors.wikilinks = true;
			mockSettings.fieldsSettings.contributors.wikilinks = true;
			mockSettings.fieldsSettings.series.wikilinks = true;
			mockSettings.fieldsSettings.publisher.wikilinks = true;
			mockSettings.fieldsSettings.genres.wikilinks = true;
			metadataService.updateSettings(mockSettings);
		});
		...
		test("returns original values when wikilinks is disabled", () => {
			mockSettings.fieldsSettings.authors.wikilinks = false;
			metadataService.updateSettings(mockSettings);
